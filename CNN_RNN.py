...
from tensorflow.keras.layers import Embedding, Conv1D, MaxPooling1D, LSTM, Dense, Dropout, Input, Flatten
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import LabelEncoder

# Load the emotion_dataset
emotion_dataset = pd.read_csv("emotion_dataset.csv")
...
cnn_rnn_input = Input(shape=(X_train.shape[1],))
embedding_layer = Embedding(input_dim=len(tokenizer.word_index) + 1, output_dim=100)(cnn_rnn_input)
...
# Add LSTM layer
lstm_layer = LSTM(64)(maxpooling_layer)
...
model = Model(inputs=cnn_rnn_input, outputs=output_layer)
...
# Evaluate the CNN+RNN model on training data
